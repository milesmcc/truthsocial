...
describe FetchOEmbedService, type: :service do
  ...
  before do
    stub_request(:get, "https://host.example.com/provider.json").to_return(status: 404)
    stub_request(:get, "https://host.example.com/provider.xml").to_return(status: 404)
    stub_request(:get, "https://host.example.com/empty_provider.json").to_return(status: 200)
  end
  ...
  describe 'discover_provider' do
    context 'when status code is 200 and MIME type is text/html' do
      context 'Both of JSON and XML provider are discoverable' do
        before do
          stub_request(:get, 'https://host.example.com/oembed.html').to_return(
            status: 200,
        ...
        it 'returns new OEmbed::Provider for JSON provider if :format option is set to :json' do
          subject.call('https://host.example.com/oembed.html', format: :json)
          expect(subject.endpoint_url).to eq 'https://host.example.com/provider.json'
          expect(subject.format).to eq :json
        end

        it 'returns new OEmbed::Provider for XML provider if :format option is set to :xml' do
          subject.call('https://host.example.com/oembed.html', format: :xml)
          expect(subject.endpoint_url).to eq 'https://host.example.com/provider.xml'
      ...
      context 'JSON provider is discoverable while XML provider is not' do
        ...
        it 'returns new OEmbed::Provider for JSON provider' do
          subject.call('https://host.example.com/oembed.html')
          expect(subject.endpoint_url).to eq 'https://host.example.com/provider.json'
          expect(subject.format).to eq :json
        end
      end
      ...
      context 'Invalid XML provider is discoverable while JSON provider is not' do
        ...
        it 'returns nil' do
          expect(subject.call('https://host.example.com/oembed.html')).to be_nil
        end
      end
      ...
      context 'Empty JSON provider is discoverable' do
        ...
        it 'returns new OEmbed::Provider for JSON provider' do
          subject.call('https://host.example.com/oembed.html')
          expect(subject.endpoint_url).to eq 'https://host.example.com/empty_provider.json'
          expect(subject.format).to eq :json
        end
      end
