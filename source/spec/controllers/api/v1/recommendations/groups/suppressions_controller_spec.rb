...
RSpec.describe Api::V1::Recommendations::Groups::SuppressionsController, type: :controller do
  let(:user)  { Fabricate(:user) }
  let(:scopes)  { 'read:suppressions write:suppressions' }
  let(:token) { Fabricate(:accessible_access_token, resource_owner_id: user.id, scopes: scopes) }
  let(:target_account) { Fabricate(:account) }
  let(:group) { Fabricate(:group, display_name: Faker::Lorem.characters(number: 5), note: Faker::Lorem.characters(number: 5), owner_account: target_account) }
  ...
  describe "POST #create" do
    ...
    context 'authenticated' do
      ...
      it 'should create an account suppression' do
        ...
        expect(user.account.group_recommendation_suppressions.size).to eq 1
      end

      it 'should return a 404 if group does not exist' do
        post :create, params: { group_id: '123123', status_id: status.id }
      ...
      it 'should return a 404 if group does not exist' do
        post :create, params: { group_id: group.id, status_id: '123123' }
      ...
      it 'should return a 422 if duplicate entry' do
        user.account.group_recommendation_suppressions.create!(group_id: group.id, status_id: status.id)
  ...
  describe 'DELETE #destroy' do
    ...
    context 'authenticated' do
      ...
      it 'should unsuppress a group recommendation' do
        user.account.group_recommendation_suppressions.create!(group_id: group.id, status_id: status.id)
      ...
      it 'should return a 404 if group is not found provided the id' do
