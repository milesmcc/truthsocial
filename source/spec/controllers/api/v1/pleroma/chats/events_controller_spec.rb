...
RSpec.describe Api::V1::Pleroma::Chats::EventsController, type: :controller do
  render_views
  ...
  describe 'GET #index' do
    ...
    it 'returns chat messages ordered by min ID' do
      ...
      Procedure.process_chat_events
      events = ChatEvent.load_events(1, account.id, nil, nil, nil, nil, true, 5)
      parsed_events = JSON.parse(events)
      get :index, params: { limit: 2, min_id: parsed_events.third['event_id'] }

      expect(response.headers['Link'].find_link(['rel', 'next']).href).to include "http://test.host/api/v1/pleroma/chats/events?limit=2&min_id=#{parsed_events.second['event_id']}"
    end
  end
end
